...
kable(confint(misc_log_model))
```
An increase on one percent in the adjusted mortality rate is associated with an
increase of 134.32 levels of Nitric Oxide, 27.08 levels of sulfur dioxide, and
-131.93 levels of hydrocarbons. 
-
...
half1_misc <- slice(misc, 1:30)

half1_lm  <- lm(mort~log_nox + log_hc + log_so2, data=half1_misc)
summary(half1_lm)
confint(half1_lm)

new_x <- slice(misc, 31:60)

predictions <- predict(half1_lm, new_x)

predicted_results <- cbind("Predicted Values" = predictions, "Observed Values" = new_x$mort)
kable(predicted_results)

```


### f) What do you think are the reasons for using cross-validation?
Cross-validation tests the accuracy of a regression. It is a method to determine
how well a model explains a group of data. If the data were to continue, could 
the model predict the future points? It is a test for how the model would 
perform when dealing with "new" data. 
...
p1 <- ggplot(data=gender, aes(x=likes_cats, y=is.female)) +  labs(title = "Cat Preferences of Bay Area OKCupid Users by Gender", 
      x= "Likes Cats? (1 yes, 0 no)", y = "Gender")
p1 +
...
# Plot.
p1 <- ggplot(data=gender, aes(x=is_veg, y=is.female)) + labs(title = "Vegetarianism of OKCupid Users by Gender", x= "Is Vegetarian?", y = "Gender")
...
Can bisexuality predict gender?
...
gender <- gender %>%  filter(orientation != "gay") 
